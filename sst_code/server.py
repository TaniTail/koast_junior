...
class TextAnalyzer:
    ...
    def _initialize_prompt_template(self):
        ...
        self.prompt_template = (
            "def analyze_maritime_incident(input_text):\n"
            "    \"\"\"\n"
            "    해양 사고 음성 신고를 분석하고 구조화된 정보를 추출하는 시스템\n"
            "\n"
            "    입력 텍스트:\n"
            "    {input_text}\n"
            "\n"
            "    시스템 동작 방식:\n"
            "    1. 음성의 텍스트 변환 내용에서 구조 작업용 핵심 정보 추출\n"
            "    2. 부정확한 발음을 문맥에 맞게 수색구조 적절 단어로 해석\n"
            "    3. 특히 알파벳은 문맥에 맞게 해석\n"
            "\n"
            "    필수 필드 구성:\n"
            "    1. type: 항상 'INCIDENT'로 고정\n"
            "    2. property: 다음 필드들로만 구성된 객체\n"
            "       - incidentName: 신고자가 '사고명은~입니다'와 같이 직접 언급한 사고명\n"
            "       - incidentType: 신고자가 직접 언급한 사고 유형\n"
            "       - shipName: '선박명은 ~호입니다'와 같이 직접 선박명임이 언급된 경우만 포함\n"
            "       - lastknownDate: 명시적인 시각 언급 필수 (YYYY-MM-DD HH:mm:ss)\n"
            "       - lastknownPosition: 정확한 위도/경도 좌표가 언급된 경우만 {{latitude, longitude}}\n"
            "       - reportedDate: 명시적인 시각 언급 필수 (YYYY-MM-DD HH:mm:ss)\n"
            "       - shipCode: 선박 코드 종류. SHIP_CODES = {{'MMSI', 'IMO', 'RFID'}}\n"
            "       - shipId: 코드 번호가 직접 언급된 경우만 (int)\n"
            "       - personOnboard: 상태와 수가 명확히 언급된 승선인원 ID 배열\n"
            "       - description: 긴급하거나 특이한 상황만 한 문장으로 요약\n"
            "    3. task: 다음 중 하나 선택\n"
            "       - enter: 기본값. 사고 등록 진행 중이거나 유의미한 정보가 있는 경우\n"
            "       - run: 다음 키워드 발견 시 (property는 빈 객체로 반환)\n"
            "         * '사고등록을 마칩니다/완료합니다/종료합니다'\n"
            "         * '등록을 마치겠습니다'\n"
            "         * '완료하겠습니다'\n"
            "       - cancel: 다음 키워드 발견 시 (property는 빈 객체로 반환)\n"
            "         * '취소'\n"
            "         * '등록을 취소'\n"
            "         * '중단'\n"
            "\n"
            "    정보 추출의 절대 원칙:\n"
            "    1. 오직 신고자의 직접 언급만 인정:\n"
            "       - '이것은 ~입니다', '~라고 합니다'와 같은 직접적인 진술만 허용\n"
            "       - 다른 필드의 내용에서 유추 가능하더라도 직접 언급이 없으면 절대 포함 금지\n"
            "       - 불완전하거나 불확실한 정보는 필드 자체를 제외\n"
            "    2. description 작성 규칙:\n"
            "       - 긴급 상황이나 특이사항만 한 문장으로 기술\n"
            "       - 다른 필드에 포함된 정보는 제외\n"
            "       - 예시: '엔진실에서 검은 연기 발생 중' (O)\n"
            "              '남해안에서 발생한 사고임' (X)\n"
            "    3. 날짜/시간 필드 규칙:\n"
            "       - 구체적인 시간 언급이 있는 경우만 포함\n"
            "       - '방금', '조금 전' 등 상대적 시간은 제외\n"
            "    4. 위치 정보 규칙:\n"
            "       - 정확한 위도/경도 값 언급 필수\n"
            "       - '남해안', '서해상' 등 일반적 위치는 제외\n"
            "    5. 승선인원 ID 매핑 (정확한 상태와 수가 명시된 경우만):\n"
            "       - 89: '구명조끼 미착용 익수자' 또는 '구명조끼를 입지 않은 익수자'\n"
            "       - 90: '구명조끼 착용 익수자' 또는 '구명조끼를 입은 익수자'\n"
            "       - 91: '낚시어선의 인원' 또는 '낚시어선에 타고 있는 사람'\n"
            "       - 46: '전복된 어선의 인원' 또는 '뒤집힌 어선의 인원'\n"
            "       - 7: '구명정의 인원' 또는 '구명정에 탑승한 인원'\n"
            "       - 67: '위성뜰개부이' 또는 '위성부이'\n"
            "       - 48: '레저보트의 인원' 또는 '레저보트에 탑승한 인원'\n"
            "\n"
            "    출력: 다음 규칙을 따르는 JSON 객체\n"
            "    {{\n"
            "        \"type\": \"INCIDENT\",\n"
            "        \"property\": {{\n"
            "            \"incidentName\": \"거제 씨프린스호 화재사고\",  # 예시\n"
            "            \"incidentType\": \"화재\",\n"
            ...
            "            \"description\": \"엔진실에서 검은 연기 발생 중\"\n"
            "        }},\n"
            "        \"task\": \"enter\"\n"
            "    }}\n"
            "\n"
            "    RULES:\n"
            "    1. 위의 모든 규칙을 엄격히 적용하여 입력 텍스트를 분석하세요\n"
            "    2. 분석 결과를 유효한 JSON 형식으로만 응답하세요\n"
            "    3. JSON 응답 외의 다른 텍스트는 포함하지 마세요\n"
            "    4. 설명이나 주석은 제외, 꼭 순수 JSON만 응답하세요\n"
            "    \"\"\"\n"
            "    return response\n"
        )
...
class WebSocketServer:
    ...
    async def handle_client(self, websocket: websockets.WebSocketServerProtocol):
        ...
        response_headers = {
            ...
            'Content-Security-Policy': "default-src *; connect-src * ws://* wss://*; script-src 'unsafe-inline' 'unsafe-eval'",
            'Sec-WebSocket-Protocol': websocket.request_headers.get('Sec-WebSocket-Protocol', '')
        }
        
        # OPTIONS 요청 처리
        if websocket.request_headers.get('Upgrade', None) != 'websocket':
