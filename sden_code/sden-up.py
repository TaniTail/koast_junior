...
async def download_traffic_data(page, date_time, retry_count=0):
    """재시도 메커니즘이 포함된 다운로드 함수"""
    max_retries = 3
    try:
        ...
        if retry_count < max_retries:
            ...
            print(f"⏳ {wait_time}초 후 {retry_count + 1}번째 재시도...")
            await asyncio.sleep(wait_time)
            return await download_traffic_data(page, date_time, retry_count + 1)
        return False
...
async def run_all():
    async with async_playwright() as p:
        ...
        start_date = datetime(2024, 5, 20, 0, 0)
